import { CustomTypeModelFieldType } from './../_node_modules/@prismicio/client/dist/types/model/types.js';
import { stripIndent, source } from "common-tags";
import { addLine } from "./addLine.js";
import { addSection } from "./addSection.js";
import { buildFieldDocs } from "./buildFieldDocs.js";
import { buildTypeName } from "./buildTypeName.js";
import { buildUnion } from "./buildUnion.js";
import { getHumanReadablePath } from "./getHumanReadablePath.js";
function buildFieldProperty(args) {
  ...
  switch (args.field.type) {
    ...
    case "Embed": {
      ...
      code = providerTypes.length > 0 ? addLine(`${name}: prismic.EmbedField<prismic.AnyOEmbed & prismic.OEmbedExtra & (${providerTypesUnion})>`, code) : addLine(`${name}: prismic.EmbedField`, code);
      break;
    }
    case "GeoPoint": {
    ...
    case "Image": {
      if (((_d = args.field.config) == null ? void 0 : _d.thumbnails) && args.field.config.thumbnails.length > 0) {
        const thumbnailNames = buildUnion(args.field.config.thumbnails.map((thumb) => `"${thumb.name}"`));
    ...
    case "Link": {
      const variants = args.field.config && "variants" in args.field.config && Array.isArray(args.field.config.variants) && args.field.config.variants.length > 0 ? args.field.config.variants.map((variant) => `"${variant.replace(/\"/g, '\\"')}"`).join(" | ") : "never";
      switch ((_h = args.field.config) == null ? void 0 : _h.select) {
        case "document": {
          if ("customtypes" in args.field.config && args.field.config.customtypes && args.field.config.customtypes.length > 0) {
            const customTypeIDsUnion = buildUnion(args.field.config.customtypes.map((type) => `"${type}"`));
            code = addLine(`${name}: prismic.ContentRelationshipField<${customTypeIDsUnion}>;`, code);
          } else {
        ...
        case "media": {
          code = addLine(`${name}: prismic.LinkToMediaField<prismic.FieldState, ${variants}>;`, code);
          break;
        }
        default: {
          let type = `prismic.LinkField<string, string, unknown, prismic.FieldState, ${variants}>`;
          if ((_i = args.field.config) == null ? void 0 : _i.repeat) {
    ...
    case "Select": {
      const options = ((_k = (_j = args.field.config) == null ? void 0 : _j.options) == null ? void 0 : _k.map((option) => `"${option}"`)) || [];
      const optionsType = options.length ? buildUnion(options) : "string";
      const hasDefault = Boolean((_l = args.field.config) == null ? void 0 : _l.default_value);
      if (hasDefault) {
        code = addLine(`${name}: prismic.SelectField<${optionsType}, "filled">;`, code);
      } else {
        code = addLine(`${name}: prismic.SelectField${options.length > 0 ? `<${optionsType}>` : ""};`, code);
      }
      break;
    }
    case "Text": {
    ...
    case "Group": {
      const groupPathParts = args.path.filter((part) => part.model !== void 0 && "type" in part.model && part.model.type === CustomTypeModelFieldType.Group);
      ...
      if (args.path[0].model && "type" in args.path[0].model && args.path[0].model.type === "SharedSlice") {
        ...
        itemName = buildTypeName(slicePathPart.name, "Slice", variationPathPart.name, zonePathPart.name, ...groupPathParts.map((part) => part.name), args.name, "Item");
      } else {
        itemName = buildTypeName(args.path[0].name, "Document", "Data", ...groupPathParts.map((part) => part.name), args.name, "Item");
      }
      const path = [
        ...args.path,
        {
      ...
      const itemFieldProperties = buildFieldProperties({
        fields: ((_m = args.field.config) == null ? void 0 : _m.fields) || {},
    ...
    case "Slices": {
      const choiceNames = [];
      if ((_n = args.field.config) == null ? void 0 : _n.choices) {
        for (const choiceID in args.field.config.choices) {
          ...
          if (choice.type === "SharedSlice") {
            ...
            const sliceName = buildTypeName(args.path[0].name, "Document", "Data", args.name, choiceID, "Slice");
            ...
            if (choice["non-repeat"] && Object.keys(choice["non-repeat"]).length > 0) {
              ...
              let primaryCode = stripIndent`
								 ...
								 */
							`;
              primaryCode = primaryFieldProperties.code ? addLine(source`
											export interface ${primaryInterfaceName} {
												${primaryFieldProperties.code}
											}
										`, primaryCode) : addLine(`export interface ${primaryInterfaceName} {}`, primaryCode);
              auxiliaryTypes.push({
            ...
            if (choice.repeat && Object.keys(choice.repeat).length > 0) {
              ...
              let itemCode = stripIndent`
								 ...
								 */
							`;
              itemCode = itemFieldProperties.code ? addLine(source`
											export interface ${itemInterfaceName} {
												${itemFieldProperties.code}
											}
										`, itemCode) : addLine(`export interface ${itemInterfaceName} {}`, itemCode);
              auxiliaryTypes.push({
      ...
      const choiceUnionName = buildTypeName(args.path[0].name, "Document", "Data", args.name, "Slice");
...
//# sourceMappingURL=buildFieldProperties.js.map
