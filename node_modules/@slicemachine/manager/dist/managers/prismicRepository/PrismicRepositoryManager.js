...
import { PrismicRepository, PrismicRepositoryRole, PrismicRepositoryUserAgent, ChangeTypes, PushChangesLimitType, Environment, OnboardingState, PushChangesRawLimit } from "./types.js";
...
class PrismicRepositoryManager extends BaseManager {
  ...
  async readAll() {
    ...
    if (res.ok) {
      ...
      const { value: repositories, error } = decode(t.array(PrismicRepository), json);
      if (error) {
  ...
  async checkExists(args) {
    const url = new URL(`./app/dashboard/repositories/${args.domain}/exists`, API_ENDPOINTS.PrismicWroom);
  ...
  async create(args) {
    const url = new URL("./authentication/newrepository?app=slicemachine", API_ENDPOINTS.PrismicWroom);
    const body = {
    ...
    const res = await this._fetch({
      ...
      method: "POST",
      body,
      userAgent: PrismicRepositoryUserAgent.SliceMachine
      // Custom User Agent is required
    });
    const text = await res.text();
    if (!res.ok || text.length < REPOSITORY_NAME_VALIDATION.Min || text.length > REPOSITORY_NAME_VALIDATION.Max) {
  ...
  async delete(args) {
    ...
    const res = await this._fetch({
      ...
      method: "POST",
      body,
      userAgent: PrismicRepositoryUserAgent.LegacyZero
      // Custom User Agent is required
    });
    if (!res.ok) {
  ...
  async pushChanges(args) {
    ...
    try {
      ...
      switch (response.status) {
        case 202:
          return this._decodeLimitOrThrow(await response.json(), response.status, PushChangesLimitType.SOFT);
        case 204:
          return null;
        case 401:
          throw new UnauthenticatedError();
        case 403:
          return this._decodeLimitOrThrow(await response.json(), response.status, PushChangesLimitType.HARD);
        case 400:
  ...
  async fetchOnboarding() {
    ...
    if (res.ok) {
      ...
      if (error) {
        throw new UnexpectedDataError(`Failed to decode onboarding: ${error.errors.join(", ")}`);
      }
      if (value) {
        return value;
      }
  ...
  async toggleOnboardingStep(stepId) {
    ...
    if (res.ok) {
      const json = await res.json();
      const { value, error } = decode(z.object({ completedSteps: z.array(z.string()) }), json);
      if (error) {
        throw new UnexpectedDataError(`Failed to decode onboarding step toggle: ${error.errors.join(", ")}`);
      }
      if (value) {
        return value;
      }
  ...
  async completeOnboardingStep(...stepIds) {
    const repositoryName = await this.project.getRepositoryName();
    const currentState = await this.fetchOnboarding();
    const incompleteSteps = stepIds.filter((stepId) => !currentState.completedSteps.includes(stepId));
    if (incompleteSteps.length > 0) {
      for await (const stepId of incompleteSteps) {
        ...
        if (res.ok) {
          ...
          const { value, error } = decode(z.object({ completedSteps: z.array(z.string()) }), json);
          if (error) {
            throw new UnexpectedDataError(`Failed to decode onboarding step complete response: ${error.errors.join(", ")}`);
          }
          if (value) {
  ...
  async toggleOnboarding() {
    ...
    if (res.ok) {
      const json = await res.json();
      const { value, error } = decode(z.object({ isDismissed: z.boolean() }), json);
      if (error) {
        throw new UnexpectedDataError(`Failed to decode onboarding toggle: ${error.errors.join(", ")}`);
      }
      if (value) {
        return value;
      }
...
export {
  PrismicRepositoryManager
};
//# sourceMappingURL=PrismicRepositoryManager.js.map
