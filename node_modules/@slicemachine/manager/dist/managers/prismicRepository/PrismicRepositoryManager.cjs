...
class PrismicRepositoryManager extends BaseManager.BaseManager {
  ...
  async pushChanges(args) {
    ...
    try {
      ...
      const response = await this._fetch({
        url: new URL("./push", API_ENDPOINTS.API_ENDPOINTS.SliceMachineV1),
        method: "POST",
        body: requestBody,
    ...
    if (incompleteSteps.length > 0) {
      for await (const stepId of incompleteSteps) {
        ...
        if (res.ok) {
          ...
          const { value, error } = decode.decode(index.default.object({ completedSteps: index.default.array(index.default.string()) }), json);
          if (error) {
            throw new errors.UnexpectedDataError(`Failed to decode onboarding step complete response: ${error.errors.join(", ")}`);
          }
          if (value) {
  ...
  async setDefaultMasterLocale() {
    const repositoryName = await this.project.getRepositoryName();
    const url = new URL("repository/locales", API_ENDPOINTS.API_ENDPOINTS.LocaleService);
    url.searchParams.set("repository", repositoryName);
    const res = await this._fetch({
      url,
      method: "POST",
