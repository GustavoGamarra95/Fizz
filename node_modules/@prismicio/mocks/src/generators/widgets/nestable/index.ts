...
import { RepeatableMock, RepeatableMockConfig } from "./Repeatable"
...
> = {
	generate(
		...
		switch (def.type) {
			...
			case "Link":
				if (def.config?.repeat) {
					return RepeatableMock.generate(
						def,
						config?.type === "Repeatable" ? config : undefined,
					)
				}
	...
	patch(
		...
		const patched =
			...
			RepeatableMock.applyPatch(data)
